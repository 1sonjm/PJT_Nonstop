...
window.addEventListener('load', function() {
	...
	function previewFile(file)
	{
		...
		var html = '<div class="file_download"><span>' + nowTime + '</span>'
			+ '<a href="' + file.url + '" target="_blank" download="' + file.name + '"">'
			+ file.name + '<br/>' + bytesToSize(file.size) + '</a></div><hr/>';
		...
		*/
		...
		document.getElementById('fileshare_div').innerHTML += html;
		fileSelector.lastSelectedFile = false;
	}

	var FileHelper = {
		onBegin : function(file) {
			document.querySelector('#fileshare_div').innerHTML += '<progress class="file_download_progress" value="0" max="' + file.maxChunks + '"/>';
		/*
		var li = document.createElement('li');li.innerHTML = '</pre><br><progress style="display:none;" value="0"></progress><div class="circular-progress-bar c100 p25" style="margin-left: 40%;"><span class="circular-progress-bar-percentage">25%</span><div class="slice"><div class="bar"></div><div class="fill"></div></div></div>';
		...
		*/
		...
		onProgress : function(chunk) { //과정이 진행될때 중간 보고라고 생각하자
			document.querySelector('.file_download_progress').value = chunk.currentPosition || chunk.maxChunks || helper.progress.max;
		/*
		...
		onEnd : function(file) {
			var target = document.querySelector('.file_download_progress');
			setTimeout(function() {
				target.parentNode.removeChild(target);
				previewFile(file);//화면에 해당 파일 다운로드경로 출력
				if (peerConnection.isOpened) {
					btnSelectFile.disabled = false;
				}
			}, 100);
	...
	var mainContainer = document.getElementById('#fileshare_div');
	...
	var a = false;
	document.querySelector("#file_share").onclick = function(event) {
		if (a == false) {
			if (event !== true) {
