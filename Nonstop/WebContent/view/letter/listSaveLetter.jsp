							...
							<c:if test="${letter.letRecView==0}"> <!-- 삭제여부 확인 조건문 -->
                             ...
                             <c:if test="${letter.letReaddate!=null}"> <!-- 읽음여부 확인 조건문 -->
                              ...
                              <td class="inbox-small-cells">
                                  <input type="checkbox" name="chbox" value="${letter.letNo}" class="mail-checkbox">
                              </td>
                              <td class="inbox-small-cells">
                              <c:if test="${letter.letSave != '1'}"> <!-- 편지 보관여부 확인 조건문 -->
                              <i id="${letter.letNo}" save="${letter.letSave}" letNo="${letter.letNo}" class="fa fa-star"></i>
                              </c:if>
                              
                              <c:if test="${letter.letSave == '1'}"> <!-- 편지 보관여부 확인 조건문 -->
                              <%-- <i id="star" save="${letter.letSave}" letNo="${letter.letNo}" class="fa fa-star inbox-started"></i> 미아내요 못할듯--%>
                              <i id="${letter.letNo }" save="${letter.letSave}" letNo="${letter.letNo}" class="fa fa-star inbox-started"></i>
                              </c:if>
                              
                              </td>
                              <td class="view-message  dont-show" letNo="${letter.letNo}" receiveId="${letter.receiveId}" >${letter.sendId}</td>
                              <td class="view-message" letNo="${letter.letNo}" receiveId="${letter.receiveId}">${letter.letTitle}</td>
                                
                                  <%-- <c:if test="${letter.fileName !=null}">
                                  <td class="view-message  inbox-small-cells"><i class="fa fa-paperclip"></i></td>
                                  </c:if> --%>
                                  
                                  <td class="view-message  text-right" letNo="${letter.letNo}" receiveId="${letter.receiveId}">${letter.letRegdate}</td>
                              </tr>
                              </c:if>
